...
class XlwtSeting(object):
    ...
    def borders(status=1):
        ...
        双线:6，细点虚线:7大粗虚线:8，细点划线:9，粗点划线:10，
        细双点划线:11，粗双点划线:12，斜点划线:13"""
        border = xlwt.Borders()
        border.left = status
        border.right = status
        border.top = status
...
class XmindToXsl(XlwtSeting):
    ...
    def write_excel(self, result='', performer='', editionname=''):
        """生成excel文件的方法"""
        row0 = ["id", "模块", "需求名称", "测试用例名称",
                "执行步骤", "预期结果", "实际结果", "执行人员", "版本"]
                    ...
                    if suit_num != 0:
                        for k in range(suit_num):
                            ...
                            if c1 != 0:
                                ...
                                self.worksheet.write_merge(x - c1 + 1, x, 0, 0, z, style)  # 写入id
...
if __name__ == "__main__":
    names = "/Users/huminghuan/Desktop/xmind/"  # XMind路径
    xx = XmindToXsl(names)
    xx.write_excel()
